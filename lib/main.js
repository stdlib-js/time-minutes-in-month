...
function minutesInMonth( month, year ) {
	...
	if ( arguments.length === 0 ) {
		...
		if ( isDateObject( month ) ) {
			...
			throw new TypeError( format( '1QLAZ', month ) );
		}
	} else {
		if ( !isString( month ) && !isInteger( month ) ) {
			throw new TypeError( format( '1QLAX', month ) );
		}
		if ( !isInteger( year ) ) {
			throw new TypeError( format( '1QL7f', year ) );
		}
		mon = month;
		yr = year;
	}
	if ( isInteger( mon ) && (mon < 1 || mon > 12) ) {
		throw new RangeError( format( '1QLAa', mon ) );
	}
	mon = lowercase( mon.toString() );
	mins = MINUTES_IN_MONTH[ mon ];
	if ( mins === void 0 ) {
		throw new Error( format( '1QLAb', mon ) );
	}
